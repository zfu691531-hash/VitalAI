"""Application use case for the profile-memory persistence flow."""
...
from VitalAI.domains.profile_memory.services import ProfileMemoryUpdateOutcome, ProfileMemoryUpdateService
...
class ProfileMemoryFlowResult:
    """Runtime result for the typed profile-memory application flow."""

    accepted: bool
    summary: EventSummary | None
    outcome: ProfileMemoryUpdateOutcome | None
...
@dataclass
class RunProfileMemoryFlowUseCase:
    """End-to-end use case for long-term profile-memory updates."""
    ...
    _pending_outcomes: dict[str, ProfileMemoryUpdateOutcome] = field(default_factory=dict, init=False, repr=False)

    def configure_handlers(self) -> None:
        """Register the profile-memory handler on the decision core."""
        self.decision_core.register_handler(
            "PROFILE_MEMORY_UPDATE",
            self._resolve_pending_decision_message,
        )

    def run(self, event: MessageEnvelope) -> ProfileMemoryFlowResult:
        """Route a typed message through runtime and profile-memory orchestration."""
        ...
        if not accepted or summary is None:
            return ProfileMemoryFlowResult(
                accepted=False,
                summary=None,
                outcome=None,
        ...
        outcome = self.memory_service.remember(summary)
